...
function PlayerWindow:CreateWindow()
    ...
    local function createTopBar()
        ...
        function PlayerWindow:SetColor(color)
            if color == "red" then
                groupMusicConfig.blueTexture:SetTexture("interface/addons/GroupMusic/assets/red.png")
            elseif color == "green" then
                groupMusicConfig.blueTexture:SetTexture("interface/addons/GroupMusic/assets/green.png")
        ...
        function PlayerWindow:SetDefaultText(text)
            ...
            for _, t in ipairs(texts) do
                if t == groupMusicConfig.currentlyPlaying:GetText() then
        ...
        groupMusicConfig.blueTexture:SetScript("OnEnter", function()
            if groupMusicConfig.currentlyPlaying:GetUnboundedStringWidth() > groupMusicConfig.currentlyPlaying:GetWidth() then
                GameTooltip:SetOwner(groupMusicConfig, "ANCHOR_CURSOR")
                GameTooltip:SetText(groupMusicConfig.currentlyPlaying:GetText(), 1, 1, 1, true)
                GameTooltip:Show()
            end
        end)

        groupMusicConfig.blueTexture:SetScript("OnLeave", function()
            GameTooltip:Hide()
        end)
    ...
    local function createChannelTabs()
        groupMusicConfig.channelTabs = CreateFrame("Frame", nil, groupMusicConfig)
        groupMusicConfig.channelTabs:SetSize(startingWidth - 20, 40)
        groupMusicConfig.channelTabs:SetPoint("BOTTOMLEFT", groupMusicConfig, 10, -20)
        ...
        for _, channel in ipairs(core.Channel.channels) do
            local tab = CreateFrame("Button", channel, groupMusicConfig.channelTabs,
                "PanelTabButtonTemplate")
            tab:SetSize(100, 26)
            ...
            if lastFrame then
                ...
                tab:SetPoint("BOTTOMLEFT", groupMusicConfig.channelTabs, "BOTTOMLEFT", 0, -6)
            end
            lastFrame = tab
            table.insert(tabs, tab)
        end

        selectTab(GroupMusicVariables.selectedChannel or tabs[1]:GetName())
    end
